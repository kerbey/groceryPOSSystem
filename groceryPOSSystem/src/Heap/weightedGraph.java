...
public class weightedGraph<T> implements WeightedGraphInterface<T>
{
	...
	private int indexIs(T startVertex)
	{
		index=0;
	...
	public int weightIs(T fromVertex, T toVertex)
	{
		...
		{
			System.out.println(" from "+fromVertex+" to"+ toVertex);
			int position=edges[row][edges.length-1];
			if(edges[row][edges.length-1]>=0)
			{
				System.out.println("weight is "+edges[row][edges.length-1]);
				return position;
			}
		}
		return 0;
